var options = {
    ...
    labels: [
        {
            label: "Name", x: 0.5, y: 0.18, color: '#ff91ad', fontsize: 500, value: 'Dummy'
        },
        {
...
function out() {
    var jpgURLat50PercentQuality = canvas.toDataURL("image/jpeg", 0.1);
...
function thumb() {
    ...
    tCanvas.width = 150;
    tCanvas.height = (150 * options.height)/options.width;
    var ctx = tCanvas.getContext("2d");
    var img = document.querySelector('#i1');
    ctx.drawImage(img, 0, 0, 150, (150 * options.height)/options.width);
...
// populate labels
{
    ...
    options.labels.forEach(function(o,i) {
        ...
        labelsContent.appendChild(tBox);
    })
}